...
%   w_: right ascension of the ascending node
%
%% Core

function  elemts = obtelements ( planet, CY )
% Constants
...
i = 2*(find(contains(planets,planet))-1) + 1;
...
obt = elements(i,:)+elements(i+1,:)*T;

% Compute the elements
a = obt(1) * au;
e = obt(2);
inc = deg2rad(obt(3));
omega = mod(deg2rad(obt(6)),2*pi);
w_ = mod(deg2rad(obt(5)-obt(6)),2*pi);
M = mod(mod(deg2rad(obt(4)-obt(5)),2*pi),2*pi);

elemts = struct("a",a,"e",e,"inc",inc,"omega",omega,"w_",w_,"M",M);
end
